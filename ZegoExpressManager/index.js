...
var ZegoExpressManager = /** @class */ (function () {
    ...
    ZegoExpressManager.createEngine = function (profile) {
        ...
        return zego_express_engine_reactnative_1["default"].createEngineWithProfile(profile).then(function (engine) {
            console.warn('[ZEGOCLOUD LOG][Manager][createEngineWithProfile] - Create success');
            ...
            return __generator(this, function (_a) {
                ...
                if (this.localParticipant.camera || this.localParticipant.mic) {
                    zego_express_engine_reactnative_1["default"].instance()
                        ...
                        .then(function () {
                        console.warn('[ZEGOCLOUD LOG][Manager][startPublishingStream] - Publish success');
                        _this.isPublish = true;
                    });
                    zego_express_engine_reactnative_1["default"].instance()
                        .enableCamera(this.localParticipant.camera)
                        .then(function () {
                        console.warn('[ZEGOCLOUD LOG][Manager][enableCamera] - Enable success', _this.localParticipant.camera);
                    });
                    zego_express_engine_reactnative_1["default"].instance()
                        .muteMicrophone(!this.localParticipant.mic)
                        .then(function () {
                        console.warn('[ZEGOCLOUD LOG][Manager][muteMicrophone] - Mute success', !_this.localParticipant.mic);
                    });
                }
    ...
    ZegoExpressManager.prototype.enableCamera = function (enable) {
        return zego_express_engine_reactnative_1["default"].instance()
            .enableCamera(enable)
            .then(function () {
            console.warn('[ZEGOCLOUD LOG][Manager][enableCamera] - Enable success', enable);
        });
    };
    ZegoExpressManager.prototype.useFrontCamera = function (enable) {
        ...
        return zego_express_engine_reactnative_1["default"].instance()
            ...
            .then(function () {
            console.warn('[ZEGOCLOUD LOG][Manager][useFrontCamera] - Enable success', enable);
    ...
    ZegoExpressManager.prototype.setLocalVideoView = function (renderView) {
        if (!this.roomID) {
            console.error('[ZEGOCLOUD LOG][Manager][setLocalVideoView] - You need to join the room first and then set the videoView');
            return;
        }
        if (renderView === null) {
            console.error('[ZEGOCLOUD LOG][Manager][setLocalVideoView] - You need to pass in the correct element');
            return;
        }
    ...
    ZegoExpressManager.prototype.setRemoteVideoView = function (userID, renderView) {
        if (renderView === null) {
            console.error('[ZEGOCLOUD LOG][Manager][setRemoteVideoView] - You need to pass in the correct element');
            return;
        }
        if (!userID) {
            console.error('[ZEGOCLOUD LOG][Manager][setRemoteVideoView] - UserID is empty, please enter a right userID');
        }
    ...
    ZegoExpressManager.prototype.setRoomExtraInfo = function (key, value) {
        ...
        return zego_express_engine_reactnative_1["default"].instance()
            ...
            if (result.errorCode === 0) {
                console.warn('[ZEGOCLOUD LOG][Manager][setRoomExtraInfo] - Set success');
            }
    ...
    ZegoExpressManager.prototype.sendBroadcastMessage = function (message) {
        return zego_express_engine_reactnative_1["default"].instance()
            ...
            .then(function (result) {
            if (result.errorCode === 0) {
                console.warn('[ZEGOCLOUD LOG][Manager][sendBroadcastMessage] - Send success');
            }
            return result.errorCode === 0;
        });
    };
    ...
    ZegoExpressManager.prototype.leaveRoom = function () {
        console.warn('[ZEGOCLOUD LOG][Manager][leaveRoom] - Stop publishing stream');
        ...
        this.participantDic.forEach(function (participant) {
            ...
            console.warn('[ZEGOCLOUD LOG][Manager][leaveRoom] - Stop playing stream', participant.streamID);
        });
    ...
    ZegoExpressManager.prototype.onRoomUserUpdate = function (fun) {
        return zego_express_engine_reactnative_1["default"].instance().on('roomUserUpdate', function (roomID, updateType, userList) {
            console.warn('[ZEGOCLOUD LOG][Manager][onRoomUserUpdate]', roomID, updateType, userList);
            var userIDList = [];
    ...
    ZegoExpressManager.prototype.onRoomExtraInfoUpdate = function (fun) {
        return zego_express_engine_reactnative_1["default"].instance().on('roomExtraInfoUpdate', function (roomID, roomExtraInfoList) {
            // this.roomExtraInfo = roomExtraInfoList[0];
            console.warn('[ZEGOCLOUD LOG][Manager][onRoomExtraInfoUpdate]', roomID, roomExtraInfoList);
            fun(roomExtraInfoList);
        });
    };
    /// Trigger when room's state changed
    ZegoExpressManager.prototype.onRoomStateUpdate = function (fun) {
        return zego_express_engine_reactnative_1["default"].instance().on('roomStateUpdate', function (roomID, state) {
            console.warn('[ZEGOCLOUD LOG][Manager][onRoomStateUpdate]', roomID, state);
            fun(state);
        });
    };
    ...
    ZegoExpressManager.prototype.onBroadcastMessageRecv = function (fun) {
        zego_express_engine_reactnative_1["default"].instance().on('IMRecvBroadcastMessage', function (roomID, chatData) {
            console.warn('[ZEGOCLOUD LOG][Manager][onBroadcastMessageRecv]', roomID, chatData);
    ...
    ZegoExpressManager.prototype.generateStreamID = function (userID, roomID) {
        if (!userID) {
            console.error('[ZEGOCLOUD LOG][Manager][generateStreamID] - UserID is empty, please enter a right userID');
        }
        if (!roomID) {
            console.error('[ZEGOCLOUD LOG][Manager][generateStreamID] - RoomID is empty, please enter a right roomID');
        }
    ...
    ZegoExpressManager.prototype.onOtherEvent = function () {
        ...
        zego_express_engine_reactnative_1["default"].instance().on('roomUserUpdate', function (roomID, updateType, userList) {
            console.warn('[ZEGOCLOUD LOG][Manager][roomUserUpdate]', roomID, updateType, userList);
            userList.forEach(function (user) {
        ...
        zego_express_engine_reactnative_1["default"].instance().on('remoteCameraStateUpdate', function (streamID, state) {
            console.warn('[ZEGOCLOUD LOG][Manager][remoteCameraStatusUpdate]', streamID, state);
            var participant = _this.streamDic.get(streamID);
            if (participant) {
        ...
        zego_express_engine_reactnative_1["default"].instance().on('remoteMicStateUpdate', function (streamID, state) {
            console.warn('[ZEGOCLOUD LOG][Manager][remoteMicStatusUpdate]', streamID, state);
            var participant = _this.streamDic.get(streamID);
            if (participant) {
        ...
        zego_express_engine_reactnative_1["default"].instance().on('roomStateUpdate', function (roomID, state, errorCode) {
            console.warn('[ZEGOCLOUD LOG][Manager][roomStateUpdate]', roomID, state, errorCode);
        });
    };
    ZegoExpressManager.prototype.playStream = function (userID) {
        if (this.mediaOptions.includes(index_entity_1.ZegoMediaOptions.AutoPlayAudio) ||
            ...
            if (participant && participant.streamID && participant.renderView) {
                var zegoView = new zego_express_engine_reactnative_1.ZegoView(participant.renderView, zego_express_engine_reactnative_1.ZegoViewMode.AspectFit, 0);
                console.warn('[ZEGOCLOUD LOG][Manager][playStream] - Start playing stream');
    ...
    ZegoExpressManager.prototype.triggerStreamHandle = function (type, enable) {
        var _this = this;
        var _a = this.localParticipant, streamID = _a.streamID, camera = _a.camera, mic = _a.mic;
        if (enable) {
            if (!this.isPublish) {
                console.warn('[ZEGOCLOUD LOG][Manager][triggerStreamHandle] - Start publishing stream');
                zego_express_engine_reactnative_1["default"].instance()
            ...
            if (((type === 'camera' && !mic) || (type === 'mic' && !camera)) &&
                !this.mediaOptions.includes(index_entity_1.ZegoMediaOptions.PublishLocalAudio) &&
                !this.mediaOptions.includes(index_entity_1.ZegoMediaOptions.PublishLocalVideo)) {
                console.warn('[ZEGOCLOUD LOG][Manager][triggerStreamHandle] - Stop publishing stream');
                zego_express_engine_reactnative_1["default"].instance()
    ...
    ZegoExpressManager.prototype.triggerPreview = function (type) {
        if (this.localParticipant.renderView) {
            if (type === 'stop') {
                // Stop preview
                console.warn('[ZEGOCLOUD LOG][Manager][triggerPreview] - Stop preview', this.localParticipant.streamID);
                zego_express_engine_reactnative_1["default"].instance()
                    .stopPreview()
                    .then(function () {
                    console.warn('[ZEGOCLOUD LOG][Manager][triggerPreview] - Stop preview success');
                });
            }
            else {
                // Start preview
                console.warn('[ZEGOCLOUD LOG][Manager][triggerPreview] - Start preview', this.localParticipant.streamID);
                var zegoView = new zego_express_engine_reactnative_1.ZegoView(this.localParticipant.renderView, zego_express_engine_reactnative_1.ZegoViewMode.AspectFit, 0);
                zego_express_engine_reactnative_1["default"].instance()
                    .startPreview(zegoView)
                    .then(function () {
                    console.warn('[ZEGOCLOUD LOG][Manager][triggerPreview] - Preview success');
                });
            }
        }
    };
